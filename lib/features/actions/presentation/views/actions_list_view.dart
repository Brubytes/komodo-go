...
import 'package:komodo_go/features/actions/presentation/providers/actions_filters_provider.dart';
...
class _ActionsListViewState extends ConsumerState<ActionsListView> {
  ...
  void initState() {
    ...
    _searchController = TextEditingController(
      text: ref.read(actionsSearchQueryProvider),
    );
    _searchFocusNode = FocusNode();
    _searchQuerySubscription = ref.listenManual<String>(
      actionsSearchQueryProvider,
      (previous, next) {
  ...
  Widget build(BuildContext context) {
    ...
    final searchQuery = ref.watch(actionsSearchQueryProvider);
    final selectedTags = ref.watch(actionsTagFilterProvider);
    final templateFilter = ref.watch(actionsTemplateFilterStateProvider);
    ...
    final fallbackTags = actionsAsync.maybeWhen(
      data: (actions) => _collectTags(actions)
                  ...
                  child: _isFiltersVisible
                      ? _FiltersPanel(
                          ...
                          onTemplateFilterChanged: (value) => ref
                              .read(actionsTemplateFilterStateProvider.notifier)
                              .value = value,
                          onSelectTags: (value) => ref
                              .read(actionsTagFilterProvider.notifier)
                              .selected = value,
                          onClearTags: () =>
                              ref.read(actionsTagFilterProvider.notifier).clear(),
                        )
                  ...
                  child: _isSearchVisible
                      ? Padding(
                          ...
                          child: _SearchField(
                            ...
                            onChanged: (value) => ref
                                .read(actionsSearchQueryProvider.notifier)
                                .query = value,
                            onClear: () {
...
class _SearchField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextField(
      key: const ValueKey('actions_search'),
      focusNode: focusNode,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Gap(16),
            Text('No actions found', style: textTheme.titleMedium),
            const Gap(8),
            Text(
              message,
...
List<ActionListItem> _applyFilters(
  List<ActionListItem> actions, {
  ...
  return actions.where((action) {
    ...
    if (normalizedTags.isNotEmpty) {
      final tagMatches = action.tags.any(
    ...
    final state = action.info.state.name.toLowerCase();
    final displayTags = _displayTags(action.tags, tagNameById);
...
List<String> _collectTags(List<ActionListItem> actions) {
  final tags = <String>{};

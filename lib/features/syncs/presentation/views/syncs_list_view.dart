...
import 'package:komodo_go/features/syncs/presentation/providers/syncs_filters_provider.dart';
...
class SyncsListView extends ConsumerStatefulWidget {
  const SyncsListView({super.key});

  @override
  ConsumerState<SyncsListView> createState() => _SyncsListViewState();
}

class _SyncsListViewState extends ConsumerState<SyncsListView> {
  ...
  void initState() {
    ...
    _searchController = TextEditingController(
      text: ref.read(syncsSearchQueryProvider),
    );
    _searchFocusNode = FocusNode();
    _searchQuerySubscription = ref.listenManual<String>(
      syncsSearchQueryProvider,
      (previous, next) {
  ...
  Widget build(BuildContext context) {
    ...
    final searchQuery = ref.watch(syncsSearchQueryProvider);
    final selectedTags = ref.watch(syncsTagFilterProvider);
    final templateFilter = ref.watch(syncsTemplateFilterStateProvider);
    ...
    final fallbackTags = syncsAsync.maybeWhen(
      data: (syncs) => _collectTags(syncs)
                  ...
                  child: _isFiltersVisible
                      ? _FiltersPanel(
                          ...
                          onTemplateFilterChanged: (value) => ref
                              .read(syncsTemplateFilterStateProvider.notifier)
                              .value = value,
                          onSelectTags: (value) => ref
                              .read(syncsTagFilterProvider.notifier)
                              .selected = value,
                          onClearTags: () =>
                              ref.read(syncsTagFilterProvider.notifier).clear(),
                        )
                  ...
                  child: _isSearchVisible
                      ? Padding(
                          ...
                          child: _SearchField(
                            ...
                            onChanged: (value) => ref
                                .read(syncsSearchQueryProvider.notifier)
                                .query = value,
                            onClear: () {
                              _searchController.clear();
                              ref.read(syncsSearchQueryProvider.notifier).query =
                                  '';
                            },
                          ),
                        )
...
class _SearchField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextField(
      key: const ValueKey('syncs_search'),
      focusNode: focusNode,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Gap(16),
            Text('No syncs found', style: textTheme.titleMedium),
            const Gap(8),
            Text(
              message,
...
List<ResourceSyncListItem> _applyFilters(
  List<ResourceSyncListItem> syncs, {
  ...
  return syncs.where((sync) {
    switch (templateFilter) {
      case TemplateFilter.exclude:
        if (sync.template) return false;
    ...
    if (normalizedTags.isNotEmpty) {
      final tagMatches = sync.tags.any(
    ...
    final name = sync.name.toLowerCase();
    final repo = sync.info.repo.toLowerCase();
    final branch = sync.info.branch.toLowerCase();
    final linkedRepo = sync.info.linkedRepo.toLowerCase();
    final provider = sync.info.gitProvider.toLowerCase();
    final path = sync.info.resourcePath.join(' ').toLowerCase();
    final state = sync.info.state.name.toLowerCase();
    final displayTags = _displayTags(sync.tags, tagNameById);
...
List<String> _collectTags(List<ResourceSyncListItem> syncs) {
  final tags = <String>{};
  for (final sync in syncs) {
    for (final tag in sync.tags) {
